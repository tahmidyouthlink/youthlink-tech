                ...
                {allWork?.map((work, index) => <div key={index}>
                    <div className="mx-auto space-y-6 rounded-xl bg-white px-8 pb-8 pt-4 font-sans shadow-lg dark:bg-[#18181B]">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-3 lg:gap-6 text-sm md:text-base">
                            <button onClick={() => handleDelete(work?._id)} className="rounded bg-[#DD2955] px-2 py-1 lg:px-5 lg:py-1.5 font-medium text-white duration-300 hover:scale-105 hover:bg-red-800 text-xs lg:text-sm flex gap-2 justify-center items-center">Delete <MdOutlineDelete className='text-xl' /></button>
                            <Link href={`/dashboard/update/${work?._id}`}><button className="text-[#DD2955] border lg:py-1.5 px-2 py-1 lg:px-5 rounded border-[#DD2955] text-xs lg:text-sm hover:scale-105 duration-300 font-medium flex gap-2 justify-center items-center">Update <FaRegEdit className='text-lg' /></button></Link>
                            <button onClick={() => open(work?._id)} className="hover:text-sky-500"><FaEye /></button>
                            <div className={`fixed z-[100] flex items-center justify-center ${openModal ? 'opacity-1 visible' : 'invisible opacity-0'} inset-0 bg-black/20 backdrop-blur-sm duration-100`}>
                                <div className={`absolute max-w-md max-h-[80vh] overflow-y-auto rounded-lg bg-white mx-4 md:mx-0 p-3 pb-5 text-center drop-shadow-2xl dark:bg-gray-800 dark:text-white ${openModal ? 'scale-1 opacity-1 duration-300' : 'scale-0 opacity-0 duration-150'} `}>
                                    ...
                                    <p className="mt-4 text-sm text-center">
                                        <h1 className='text-red-700 font-bold py-1'>About The Project</h1>
                                        <p className='text-neutral-400 px-8'>{details?.aboutTheProject}</p>
                                    </p>
                                    <p className="mt-4 text-sm text-center">
                                        <h1 className='text-red-700 font-bold py-1'>Our Solution</h1>
                                        <p className='text-neutral-400 px-8'>{details?.ourSolution}</p>
                                    </p>
                                    <p className="mt-4 text-sm text-center">
                                        <h1 className='text-red-700 font-bold py-1'>The Results</h1>
