...
const AddContent = () => {
  ...
  const onSubmit = async (data) => {
    ...
    // If the image is new, upload it
    if (croppedImage2) {
      const croppedBlob = await fetch(croppedImage2).then((res) => res.blob());
      imageUrl2 = await uploadImageToImgbb({ file: croppedBlob });
      if (!imageUrl2) {
        toast.error('Image upload failed, cannot proceed.');
        return;
      }
    ...
    try {
      const response = await axiosPublic.post("/addMarketingContent", contentData);
              ...
              <div className="flex items-start">
                <div className="ml-3 flex-1">
                  <p className="text-base font-bold text-gray-900">
                    Marketing content successfully added!
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                    Your marketing content has been saved and is now live.
                  </p>
                </div>
              </div>
            </div>
