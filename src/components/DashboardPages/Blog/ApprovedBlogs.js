...
const ApprovedBlogs = () => {
    ...
    const toggleShowMore = () => {
        if (showMoreData === 5) {
            setShowMoreData(checkedBlog.length);
    ...
    return (
        <PrivateRoute>
            {checkedBlog?.length > 0 ? <div>
                <h1 className='mt-8 my-4 px-10 font-semibold'>Approved Blogs</h1>
                <div className="max-w-screen-xl mx-auto px-6">
                    <div className="overflow-x-auto">
                        <table className="table">
                            ...
                            <tbody>
                                {/* row 1 */}
                                {checkedBlog?.slice(0, showMoreData).map((work, index) => (
                                    <tr key={index}>
                                        <td>
                ...
                <h1 className='mt-8 my-4 px-10 font-semibold'>Approved Blogs</h1>
                <h1 className='text-center my-8'>There is no approved blogs</h1>
            </div>}
        </PrivateRoute>
    );
};
